...
if [ $VENDOR == "SAMSUNG" ] && [ -x /system_ext/bin/sg_write_buffer -o -x /system_ext/bin/sg_write_buffer_msi ] ; then
	...
	elif [ "$MODEL" == "MT256GAXAU4U2281" -o "$MODEL" == "MT128GAXAU2U22" ] ; then
		if [ "$REV" != "0107" ] ; then
			UPGRADE_NEEDED=1
		fi
	...
	if [ "$MODEL" == "C2G072" ] ; then
		UFS_SIZE="128"
		if [ "$REV" == "0010" -o  "$REV" == "0011" -o "$REV" == "0012" ] ; then
			UPGRADE_NEEDED=1
		fi
		FW_FILE=/vendor/etc/motorola/firmware/$VENDOR-C2G072-0013.fw
	fi
	if [ "$MODEL" == "C2G074" ] ; then
		...
		FW_FILE=/vendor/etc/motorola/firmware/$VENDOR-C2G074-0013.fw
	fi
